 ...
 *
 * Local adaptations from the source component:
 * - focus ring uses the theme `--ring`/primary token instead of a hardcoded blue
 * - `NodeJS.Timeout` swapped for `ReturnType<typeof setTimeout>` so the file
 *   type-checks without @types/node in scope
 */
export const RadialScrollGallery = forwardRef<HTMLDivElement, RadialScrollGalleryProps>(
  (
    ...
    // Total height required for the pinned container:
    // (visible circle height) + (half item height) + buffer, so items aren't clipped by the mask.
      ...
      >
        ...
        >
          <ul
            ref={containerRef}
            className={`
              absolute left-1/2 m-0 list-none -translate-x-1/2 p-0 will-change-transform
          ...
          >
            {childrenNodes.map((child, index) => {
              ...
              return (
                ...
                >
                  {/*
                    A generic div with role="button" rather than <button> so interactive
                    children (like <Link>) don't create invalid HTML nesting.
                  */}
                  <div
                    role="button"
